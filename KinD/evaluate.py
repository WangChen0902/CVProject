...
### change the ratio to get different exposure level, the value can be 0-5.0
    ...
    fusion = restoration_r
# fusion with the original input to avoid over-exposure
    fusion2 = decom_i_low*input_low_eval + (1-decom_i_low)*fusion
    # print(fusion2.shape)
    save_images(os.path.join(sample_dir, '%s.jpg' % (name)), fusion2)
    # save_images(os.path.join(sample_dir, '%s_i.jpg' % (name)), adjust_i)
    # save_images(os.path.join(sample_dir, '%s_i.jpg' % (name)), decom_i_low)
    # save_images(os.path.join(sample_dir, '%s.jpg' % (name)), fusion)
    # save_images(os.path.join(sample_dir, '%s_r.jpg'% (name)), decom_r_low)
    # save_images(os.path.join(sample_dir, '%s_rr.jpg'% (name)), restoration_r)
